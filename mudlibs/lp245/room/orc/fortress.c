void extra_reset() {
  ...
  if (!present("orc")) {
    while(i<8) {
      ...
      call_other("room/orc/orc_vall", "get_chats"));
...
status north() {
  ...
  call_other(this_player(), "move_player", "north#room/orc/orc_treasure");
...
status south() {
  call_other(this_player(),"move_player", "south#room/orc/orc_vall");
