...
// Time zone code, by Docz
    ...
    divider();
    printf("%-12s%-12s%-6s%-22s%-22s",
	"Name", "Position", "Idle", "Location", "IP Address\n");
    divider();
	...
	field = (string) list[i] -> query(a_position);
	if (!field)
	  field = "(none)";
	line += sprintf("%-12s", field);
	...
	field = (string) list[i] -> query(a_ip_address);
	if (!field)
	  field = "(none)"; /* Shouldn't happen. */
	line += sprintf("%-22s", field);
	write( line + "\n" );
    }
    divider();
