		    ...
		    mixed target, string msg_target, string extra)
{
  ...
  if (target)
    { 
       target_name = (string)target->query_cap_name();
  ...
  if (toself)
    msg_others = replace_string(msg_others, "{n}", act_objective+"self");
  else
   msg_others = replace_string(msg_others,"{n}", target_name);
  if (toself)
    msg_self = replace_string(msg_self,"{n}","yourself");
  else
