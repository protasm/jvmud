...
import io.github.protasm.jvmud.maintenance.update.InstallationServers;
...
final class EngineLauncher {
    ...
    static LaunchOptions parseLaunchOptions(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            else if (java.util.Set.of("--bind", "--port", "--admin-port", "--state-dir").contains(arg)) {
            ...
            else mudlibs.add(MudlibSpec.fromConfig(resolveLaunchConfigFile(Path.of(arg), launchRoot())));
        }
        ...
        return new LaunchOptions(java.util.List.copyOf(mudlibs), port, bind, help, trace, adminPort, state);
    }
    ...
    private static String usage() {
        return "Usage: scripts/jvmud-start [--bind <address>] [--port <player-port>] [--admin-port <port>] "
                + "[--state-dir <private-directory>] [--trace-startup-loads] [<mudlib-config-or-name>...]\n"
    ...
    record LaunchOptions(java.util.List<MudlibSpec> mudlibs, int port, String bindAddress,
            boolean help, boolean traceStartupLoads, int adminPort, Path stateDirectory) {}
}
