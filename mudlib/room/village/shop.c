...
buy(item) {
  ...
  call_other("room/village/store", "buy", item);
...
do_sell(ob) {
  ...
  call_other("room/village/store", "store", ob);
...
north() {
  ...
  call_other(this_player(), "move_player", "north#room/village/store");
...
value(item) {
  ...
  if (!name_of_item) {
    if (call_other("room/village/store", "value", item))
      return 1;
