...
advance(arg) {
  ...
  if (next_level == 20 && call_other("room/village/quest_room", "count", 0))
    return 1;
...
list_quests(num) {
  ...
  if (num && (sscanf(num, "%d", qnumber) == 1))
    call_other("room/village/quest_room", "list", qnumber);

  else
    call_other("room/village/quest_room", "count", 0);
...
south() {
  ...
  call_other(this_player(), "move_player", "south#room/village/adv_inner");
